...
class MultiViewMVTec3D(Dataset):
    ...
    def load_dataset(self):
        ...
        for defect_type in defect_types:
            if defect_type == "good":
                rgb_paths = glob.glob(
                    os.path.join(self.img_path, defect_type, "rgb") + "/*.png"
                )
                rgb_paths.sort()
                ...
                gt_paths = glob.glob(
                    os.path.join(self.img_path, defect_type, "gt") + "/*.png"
                )
                rgb_paths.sort()
                gt_paths.sort()
                xyz_roots = [s.replace("rgb", "xyz")[:-4] for s in rgb_paths]
    ...
    def __getitem__(self, idx):
        ...
        # tiff中，三通道分别代表xyz的数值
        resized_organized_pc, features, view_images, view_positions = self.read_xyz(
            xyz_root
        )
        ...
        return (
            (img, resized_organized_pc, features, view_images, view_positions),
            gt[:1],
            label,
        )
    ...
    def read_xyz(self, xyz_root):
        ...
        view_images = [
            self.rgb_transform(Image.open(image_path).convert("RGB"))
            for image_path in view_image_paths
        ]
        view_positions = [
            np.load(position_path) for position_path in view_position_paths
        ]
...
def get_data_loader(split, class_name, img_size, dataset_path, batch_size=1):
    dataset = MultiViewMVTec3D(
        split=split, class_name=class_name, img_size=img_size, dataset_path=dataset_path
    )
    if split in ["train"]:
...
def unorganized_data_to_organized(organized_pc, none_zero_data_list):
    ...
    for none_zero_data in none_zero_data_list:
        full_data = np.zeros(
            (unorganized_pc.shape[0], none_zero_data.shape[1]),
            dtype=none_zero_data.dtype,
        )
        full_data[nonzero_indices, :] = none_zero_data
        full_data_reshaped = full_data.reshape(
            (
                organized_pc_np.shape[0],
                organized_pc_np.shape[1],
                none_zero_data.shape[1],
            )
        )
        full_data_tensor = (
            torch.tensor(full_data_reshaped).permute(2, 0, 1).unsqueeze(dim=0)
        )
...
def denormalization(x):
    x = (((x.transpose(1, 2, 0) * IMAGENET_STD) + IMAGENET_MEAN) * 255.0).astype(
        np.uint8
    )
    return x
